...
# set.seed(369)
# rate_parameter(lambda = 0.13, LP = data.frame(LP), time.point = 1)
#Rate 0.130 -> S(1)~0.8634; C=0.62; D=0.69
#Rate 0.029 -> S(1)~0.9661; C=0.61; D=0.61
#This engulfs range of survival probabilities of interest
lambda_1yr<- seq(0.029, 0.130, length.out = 20)
...
for(j in 1:length(lambda_1yr)){
  for(i in 1:N.SIM){
    ...
    LP<- data.frame(id = 1:N.OBS,
                    value = log(KDRI))
    OUT<- rate_parameter(lambda = lambda_1yr[j], LP = LP, time.point = 1)
    St[i,j]<- OUT$St
...
# #Plot of lambdas and the corresponding probability of survival at 1 year
# plot(lambda_1yr, colMeans(St), type = "b", pch = 20, xlab = "lambda_c", ylab = "S(1)")
# grid()
...
samplesurv_1yr<- data.frame(surv = St_Interpolate$x,
                            lambda = St_Interpolate$y#,
                            # c = C_Interpolate$y,
                            # d = D_Interpolate$y
                            )
write.csv(samplesurv_1yr, paste0(folder, "lambdas_1yr.csv"), row.names = F)
...
for (i in 1:N.SIM) {
  ...
  X.sim$cens[X.sim$eventtime > 1]<- 0
  X.sim$eventtime<- X.sim$eventtime
  X.sim$eventtime[X.sim$eventtime > 1]<- 1
  fit.survfit<- survfit(Surv(eventtime, status)~1, data = X.sim)
  cens[i,1]<- summary(fit.survfit, times = 0.99999999)$sur
}
